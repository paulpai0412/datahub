      ...
      switch (type) {
        ...
        case "get_state": {
          ...
          return {
            ...
            // Reuse the existing pending UI snapshot; the trusted DataHub Host
            // checks the exact native request before accepting a Task Decision.
            extensionUiRequests: [...this.pendingUiRequests.values()],
          };
        }
...
export async function startRpcSession(
  ...
  const starting = (async () => {
      ...
      resourceLoaderOptions: subagentResources
        ? {
            ...
            additionalExtensionPaths:
              subagentResources.loadExtensions && process.env.PI_WEB_MCP_ADAPTER
    ...
    const startupModel =
      restoredModel &&
      services.modelRuntime.hasConfiguredAuth(restoredModel.provider)
        ? restoredModel
        : initial?.model;
