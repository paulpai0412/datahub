...
function setup(t, handler = async () => {}) {
  ...
  const inner = {
    ...
    _tryExecuteExtensionCommand:
      AgentSession.prototype._tryExecuteExtensionCommand,
  ...
  test(`Stop unwinds a command waiting on ${method} and closes its UI`, async (t) => {
    ...
    assert.ok(
      events.some(
        (event) =>
          event.id === request.id &&
          (event.type === "extension_ui_closed" || event.closed),
      ),
    );
    ...
    assert.equal(
      replay.some((event) => event.id === request.id),
      false,
    );
    ...
    await nextTurn();
    const nextRequest = events.findLast(
      (event) => event.method === method && !event.closed,
    );
...
test("get_state exposes the same pending native UI as SSE replay and clears it after response", async (t) => {
  const { wrapper, ui, events } = setup(t);
  const waiting = ui.input("DataHub task decision", '{"runUrn":"fixture"}');
  const request = events.at(-1);
  const state = await wrapper.send({ type: "get_state" });
  assert.deepEqual(state.extensionUiRequests, [request]);
  const replay = [];
  wrapper.onEvent((event) => replay.push(event))();
  ...
  assert.equal(await waiting, "Continue");
  assert.deepEqual(
    (await wrapper.send({ type: "get_state" })).extensionUiRequests,
    [],
  );
});
...
test("normal cancel and caller abort keep UI defaults and notify all subscribers", async (t) => {
  ...
  const confirm = ui.confirm("Confirm", "Continue?", {
...
test("Stop cancels a custom UI factory that has not mounted yet", async (t) => {
  ...
  assert.equal(
    events.some((event) => event.method === "custom"),
    false,
  );
  assert.equal(wrapper.activeCustomUis.size, 0);
  await assert.rejects(ui.select("Reopen after Stop", ["A"]), {
    name: "AbortError",
  });
});
