...
const themeOptionsSource = await readFile(
...
test("opens one settings panel from direct sidebar shortcuts", () => {
  ...
  const shortcut =
    /<SettingsSectionIcon\s+section=\{section\}\s+size=\{14\}\s+strokeWidth=\{2\}\s*\/>\s*<span>\{label\}<\/span>/;
  assert.match(shellSource, shortcut);
  assert.doesNotMatch(
    shellSource.match(shortcut)[0].replace("size={14}", "size={15}"),
    shortcut,
  );
  assert.match(
    shellSource,
    /<SettingsSectionIcon\s+section="general"\s+size=\{14\}\s+strokeWidth=\{2\}\s*\/>/,
  );
  assert.doesNotMatch(
    shellSource,
...
test("keeps every requested configuration surface inside the settings panel", () => {
  ...
  for (const component of [
    ...
    assert.match(panelSource, new RegExp(`<${component}\\s+embedded\\b`));
  }
  const mcpHost = /sectionHost\(\s*"mcp",\s*<McpConfig\s+sessionId=\{sessionId\}\s+onReloaded=\{onSessionReloaded\}/;
  assert.match(panelSource, mcpHost);
  assert.doesNotMatch(panelSource.match(mcpHost)[0].replace("onSessionReloaded", "wrongHandler"), mcpHost);
  assert.match(panelSource, /id: "mcp", label: "MCP", requiresProject: false/);
});
...
test("groups chat display controls together without row backgrounds", () => {
  ...
  assert.equal(
    (chatSection.match(/className="settings-chat-option(?: |")/g) ?? []).length,
    4,
  );
  assert.equal((chatSection.match(/<ConfigSwitch/g) ?? []).length, 2);
  for (const key of [
    "thinkingExpandedDefault",
    "chatContentWidth",
    "chatContentFontSize",
    "quoteSelection",
  ]) {
  ...
  const chatOptionStyles =
    cssSource.match(/\.settings-chat-option \{[\s\S]*?\}/)?.[0] ?? "";
...
test("keeps password authentication to one login field and one settings action", () => {
  ...
  assert.match(
    panelSource,
    /fetch\("\/api\/web-auth", \{ method: "DELETE" \}\)/,
  );
  assert.match(panelSource, /t\("auth\.logOut"\)/);
  assert.match(
    loginSource,
    /className="web-login-composer"[\s\S]*?type="password"[\s\S]*?<button type="submit"/,
  );
  assert.match(
    globalCssSource,
    /\.web-login-composer \{[\s\S]*?display: flex;[\s\S]*?border-radius: 14px/,
  );
});
