...
test("tracks the visual viewport while the software keyboard is open", () => {
  ...
  const height =
    /height:\s*DATAHUB_EMBED\s*\?\s*"calc\(var\(--app-viewport-height, 100dvh\) - 56px\)"\s*:\s*"var\(--app-viewport-height, 100dvh\)"/;
  assert.match(appShellSource, height);
  assert.doesNotMatch(
    appShellSource.replaceAll(" - 56px)", " - 12px)"),
    height,
  );
  assert.match(
    appShellSource,
...
test("contains chat content and inputs within the mobile viewport", () => {
  ...
  assert.match(
    chatInputSource,
    /flex: compact \? "none" : 1,\s*minWidth: 0,\s*width: "100%",/,
  );
});
