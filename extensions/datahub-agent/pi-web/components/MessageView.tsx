...
export function getModelDisplayName(
  ...
  const configured = Object.entries(modelNames ?? {}).flatMap(([key, name]) => {
    ...
    return separator > 0 &&
      key.slice(0, separator).toLowerCase() === normalizedProvider
      ? [{ id: key.slice(separator + 1).toLowerCase(), name }]
      : [];
  });
  return (
    configured.find((model) => model.id === normalizedResponse)?.name ??
    configured.find((model) => normalizedResponse.endsWith(`/${model.id}`))
      ?.name ??
    Object.entries(modelNames ?? {}).find(
  ...
  const imageBlocksNode = imageBlocks.length > 0 && (
    ...
    >
      {imageBlocks.map((img, i) => {
        ...
        // SAFETY: these optional strings are the SDK's legacy image contract,
        // absent from the local nested ImageContent type; only used as a data URL.
                ...
                {commandArgs && (
                  <span
                    style={{
                      color: "var(--text)",
                      fontSize:
                        "calc(14px + var(--chat-font-size-offset, 0px))",
                      lineHeight: 1.6,
                      whiteSpace: "pre-wrap",
                      wordBreak: "break-word",
                      minWidth: 0,
...
function isSubagentToolDetails(value: unknown): value is SubagentToolDetails {
  ...
  return (
    details.kind === "pi-web-subagent" && typeof details.sessionId === "string"
  );
}
...
function imageSource(img: ImageContent): string {
  // SAFETY: legacy SDK images have optional flat data/mimeType strings; this
  // compatibility view is used only to form an image URL, not executable content.
  const flat = img as unknown as { data?: string; mimeType?: string };
  if (img.source) {
