...
test("keeps chat scroll positions in page memory by session id", () => {
  const storage = /useRef\(\s*new Map<string,\s*ChatScrollPosition>\(\),?\s*\)/;
  assert.match(source, storage);
  assert.doesNotMatch(source.replace(/Map<string,/g, "Map<number,"), storage);
  assert.match(
    source,
    /sessionScrollPositionsRef\.current\.set\(sessionId, position\)/,
  );
  const restore =
    /initialScrollPosition=\{\s*selectedSession\s*\?\s*\(?sessionScrollPositionsRef\.current\.get\(\s*selectedSession\.id,?\s*\)\s*\?\? null\)?\s*:\s*null\s*\}/;
  assert.match(source, restore);
  assert.doesNotMatch(
    source.replace(/selectedSession\.id/g, "selectedSession.otherId"),
    restore,
  );
  assert.match(
    source,
...
test("New restores the draft after session navigation and workspace auto-restore", async (t) => {
  ...
  for (const [rememberedCwd, embedded] of [
    "/draft-project",
    "/draft-project-worktree",
  ].flatMap((cwd) => [false, true].map((embedded) => [cwd, embedded]))) {
    await t.test(
      `remembered session cwd: ${rememberedCwd}, embedded=${embedded}`,
      async () => {
