...
test("renders temporary notices once at the top right of the chat column", () => {
  ...
  assert.match(
    source,
    /position: "absolute",\s*top: 12,\s*left: 0,\s*right: isMobile \? 0 : CHAT_MINIMAP_WIDTH,[\s\S]*?justifyContent: "flex-end",[\s\S]*?<NoticeShelf\s+notices=\{notices\}\s+floating\s+onPauseChange=\{setNoticePaused\}\s*\/>/,
  );
});
