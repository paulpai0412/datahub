...
function slashDisplayName(command: SlashCommandPaletteItem): string {
  return command.source === "skill" && command.name === "skill:datahub-etl"
    ? "datahub_etl"
    : command.name;
}
...
function slashMatchRank(
  ...
  const name = slashDisplayName(command).toLowerCase();
...
export function getUserMessageDraftImages(
  ...
  return message.content.flatMap((block) => {
    ...
    // SAFETY: older session entries have flat fields absent from the current type;
    // both values are narrowed to strings below before creating a draft image.
...
export function modelSupportsImageInput(
  ...
  modelList:
    | { id: string; name: string; provider: string; input?: string[] }[]
    | undefined,
): boolean {
...
export function ModelScopeWarningBanner({ warnings }: { warnings?: string[] }) {
  ...
  return (
    <ModelNoticeBanner
      ...
      title={
        warnings.length > 1
          ? t("chat.modelScopeWarnings")
          : t("chat.modelScopeWarning")
      }
      body={warnings.join("\n")}
    />
  );
...
export const ChatInput = forwardRef<ChatInputHandle, Props>(function ChatInput(
  ...
  const slashQuery =
    !compact && value.startsWith("/") && !/\s/.test(value.slice(1))
    ...
    return [...commands]
      .filter((command) => {
        ...
        return (
          ...
          slashDisplayName(command).toLowerCase().includes(slashQuery) ||
    ...
    (e: KeyboardEvent<HTMLTextAreaElement>) => {
      ...
      if (sendShortcut) {
        ...
        if (isStreaming && (onSteer || onFollowUp)) {
          sendQueued(
            (e.altKey && onFollowUp) || !onSteer ? "followup" : "steer",
          );
        } else {
          handleSend();
        }
  ...
  const handlePaste = useCallback(
    (e: React.ClipboardEvent<HTMLTextAreaElement>) => {
      ...
      const nextValue =
        ta.value.slice(0, start) + markdown + ta.value.slice(ta.selectionEnd);
      e.preventDefault();
      valueRef.current = nextValue;
      setValue(nextValue);
      setHistoryMenuOpen(false);
          ...
          (() => {
            ...
            return (
              <ModelNoticeBanner
                ...
                body={t("chat.imageNotSupportedBody", {
                  model: entry?.name || model?.modelId || "",
                })}
              ...
              >
                ...
                {onFollowUp && (
                  <button
                    ...
                    aria-keyshortcuts={
                      isMobile
                        ? "Control+Alt+Enter Meta+Alt+Enter"
                        : "Alt+Enter"
                    }
                    style={{
                      display: "flex",
                ...
                {onSoundToggle !== undefined && (
                  <button
                    ...
                    style={{
                      ...
                      transition:
                        "background 0.12s, color 0.12s, opacity 0.12s",
                    }}
                    onMouseEnter={(e) => {
                      e.currentTarget.style.background = "var(--bg-hover)";
                      e.currentTarget.style.color = "var(--text)";
                    ...
                    onMouseLeave={(e) => {
                      ...
                      e.currentTarget.style.color = soundEnabled
                        ? "var(--text-muted)"
                        : "var(--text-dim)";
                      e.currentTarget.style.opacity = soundEnabled
                        ? "1"
                        : "0.55";
                    }}
                  >
