...
export function getSessionListIndices(
  ...
  const overscan = 8;
  const visibleCount =
    Math.ceil((viewportHeight || 600) / SESSION_LIST_ITEM_HEIGHT) +
    overscan * 2;
  const start = Math.max(
    0,
    Math.min(
      Math.floor(scrollTop / SESSION_LIST_ITEM_HEIGHT) - overscan,
      count - visibleCount,
    ),
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {DATAHUB_EMBED ? <strong>History</strong> : <PiWebTitle />}
          <div style={{ display: "flex", gap: 6 }}>
            ...
            {DATAHUB_EMBED && (
              <button
                type="button"
                className="datahub-drawer-close"
                data-datahub-sidebar-close
                onClick={onClose}
                aria-label="Close History and Workspace"
              >
                ×
              </button>
            )}
