...
import {
  DataHubHostBridge,
  isDataHubHostRequest,
} from "./DataHubIngestionBridge";
...
export function ChatWindow({
  ...
  const captureQuotedSelection = useCallback(() => {
    ...
    const sourceEntryId = [ancestor, start, end]
      .map(
        (element) =>
          element?.closest<HTMLElement>('[data-message-role="assistant"]')
            ?.dataset.entryId,
      )
    ...
    setQuotedSelection({
      ...
      left: Math.max(
        64,
        Math.min(window.innerWidth - 64, rect.left + rect.width / 2),
      ),
      sourceEntryId,
    });
  ...
  useEffect(() => {
    ...
    quoteChatInputRef.current?.insertIfEmpty(
  ...
  useEffect(() => {
    if (loading || error || !initialPrompt || initialPromptSentRef.current)
      return;
  ...
  const [pendingSearchScroll, setPendingSearchScroll] =
    useState<Props["searchTarget"]>(null);
  const searchMessage =
  ...
  const visibleMessages = messages.filter(
    (m) => isMessageGroupAnchor(m) || m.role === "assistant",
  );
  ...
  return (
    ...
    >
      ...
      <div className="relative flex min-h-0 min-w-0 flex-1 overflow-hidden">
        {extensionDialog &&
          (DATAHUB_EMBED && isDataHubHostRequest(extensionDialog) ? (
            <DataHubHostBridge
                ...
                >
                  {(() => {
                        ...
                        <div
                          ...
                          ref={
                            options.attachRef === false
                              ? undefined
                              : attachVisibleRef(idx, currentRefIdx)
                          }
                        >
                          {view}
                        </div>
                    ...
                    for (let idx = 0; idx < messages.length; ) {
                      ...
                      const finalProcessEnd = finalAssistant.content.indexOf(
                        finalSplit.answerBlocks[0],
                      );
      ...
      {quoteSelectionEnabled &&
        ...
        createPortal(
          <div
            ...
            aria-label={t(
              quoteInputOpen ? "chat.newQuoteChat" : "chat.askSelection",
            )}
            style={{
              position: "fixed",
      ...
      <div className="relative shrink-0">
        {isEmptyNew && DATAHUB_EMBED && (
          <div className="datahub-welcome">
            <h2>From a question to a reviewed change.</h2>
            <p>
              Explore your workspace, inspect files, and work with your
              configured model.
            </p>
          </div>
        )}
      ...
      {isEmptyNew && !DATAHUB_EMBED && <div className="min-h-0 flex-1" />}
    </div>
  );
}
        ...
        >
          ...
          >
            <div style={{ flex: 1, minWidth: 0 }}>
              <div
                style={{ color: "var(--text)", fontSize: 14, fontWeight: 650 }}
              >
                {request.title}
              </div>
              <div
