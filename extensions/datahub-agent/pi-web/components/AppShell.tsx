...
import { DATAHUB_EMBED, THEME_OPTIONS } from "@/lib/theme";
...
export function AppShell() {
  ...
  useEffect(() => {
    try {
      setQuoteSelectionEnabled(
        localStorage.getItem("pi-quote-selection-enabled") === "true",
      );
    } catch {
  ...
  const [sidebarOpen, setSidebarOpen] = useState(!DATAHUB_EMBED);
  ...
  useLayoutEffect(() => {
    if (!DATAHUB_EMBED) return;
    const panel = sidebarResizer.panelRef.current;
    if (sidebarOpen && !sidebarWasOpen.current) {
      panel
        ?.querySelector<HTMLButtonElement>("[data-datahub-sidebar-close]")
        ?.focus();
    } else if (
      !sidebarOpen &&
      sidebarWasOpen.current &&
  ...
  }, [sidebarOpen, sidebarResizer.panelRef]);
  useEffect(() => {
    if (DATAHUB_EMBED && (rightPanelOpen || settingsSection))
      setSidebarOpen(false);
  }, [rightPanelOpen, settingsSection]);
  ...
  const [pendingQuotePrompt, setPendingQuotePrompt] = useState<{
    sessionId: string;
    text: string;
  } | null>(null);
  ...
  const handleSidebarToggle = useCallback(
    (event?: React.MouseEvent<HTMLButtonElement>) => {
      if (DATAHUB_EMBED && event)
        sidebarTriggerRef.current = event.currentTarget;
      if (DATAHUB_EMBED) setRightPanelOpen(false);
      if (isMobile || DATAHUB_EMBED) {
  ...
  useLayoutEffect(() => {
    ...
    const trigger =
      activeTopPanel === "theme" ? themeBtnRef.current : languageBtnRef.current;
    ...
    const dismissOutside = (event: Event) => {
      if (
        ...
        event.composedPath().includes(trigger)
      )
        return;
      setActiveTopPanel(null);
      const toolbar = mobileToolbarRef.current;
      if (
        ...
        (!toolbar || !event.composedPath().includes(toolbar))
      ) {
        setMobileToolbarMoreOpen(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (
        (activeTopPanel === "language" || activeTopPanel === "theme") &&
        !isMobile
      ) {
        const button =
  ...
  useEffect(() => {
    try {
      const saved = restoreTerminalTabs(
        window.sessionStorage.getItem(TERMINAL_TABS_KEY),
      );
  ...
  const handleAtMention = useCallback(
    (relativePath: string, isDir: boolean) => {
      ...
      if (DATAHUB_EMBED) setSidebarOpen(false);
      if (isMobile) {
    ...
    (
      ...
      if (!isRestore && selectedSession) {
        ...
        if (selectedSession.id === session.id && sameProject) {
          if (isMobile || DATAHUB_EMBED) setSidebarOpen(false);
          return;
        }
      }
      ...
      if ((isMobile || DATAHUB_EMBED) && !isRestore) setSidebarOpen(false);
      if (isRestore) {
  ...
  const handleAskInNewChat = useCallback(
    async (prompt: string, sourceSessionId: string, sourceEntryId: string) => {
      const result = await sendAgentCommand<{ newSessionId?: string }>(
        sourceSessionId,
        {
          type: "fork_branch",
      ...
      if (!result?.newSessionId)
        throw new Error(translate("chat.quoteForkFailed"));
  ...
  const handleTerminalClosed = (tab: TerminalTab) => {
    const replacement =
      tab.closing === "restart" ? newTerminalTab(tab.cwd) : null;
    const remaining = terminalTabs.filter((item) => item.id !== tab.id);
    setTerminalTabs((tabs) =>
      tabs.flatMap((item) =>
        item.id !== tab.id ? [item] : replacement ? [replacement] : [],
      ),
    );
    ...
    if (!replacement && !remaining.length && !fileTabs.length)
      setRightPanelOpen(false);
  };
  ...
  const handleCloseFileTab = useCallback(
    (tabId: string) => {
      ...
      setFileTabs((prev) => {
        ...
        if (next.length === 0 && terminalTabs.length === 0)
          setRightPanelOpen(false);
        return next;
      });
  ...
  const appTitle = DATAHUB_EMBED ? "Agent | DataHub" : "Pi Web";
  ...
  const sidebarContent = (
    <>
      <SessionSidebar
        onClose={DATAHUB_EMBED ? () => setSidebarOpen(false) : undefined}
    ...
    !DATAHUB_EMBED && (
      <button
        ...
        style={{
          ...
          padding: 0,
          background:
            activeTopPanel === "theme" ? "var(--bg-selected)" : "none",
          border: "none",
          borderRight: "1px solid var(--border)",
          color:
            activeTopPanel === "theme" ? "var(--text)" : "var(--text-muted)",
          cursor: "pointer",
          flexShrink: 0,
  ...
  const renderMainFileToggle = (mobile: boolean) => {
    ...
    return (
      <button
        ...
        className={DATAHUB_EMBED ? "datahub-details-toggle" : undefined}
        style={{
      ...
      >
        ...
        {DATAHUB_EMBED && <span>Details</span>}
      </button>
    );
  };
