...
test("delegates event stream readiness and hides an empty agent phase", () => {
  ...
  assert.match(
    chatWindowSource,
    /const hasStreamingContent = Boolean\(\s*streamState\.streamingMessage\?\.content\.length,?\s*\)/,
  );
  assert.match(
    chatWindowSource,
...
test("routes blocking extension requests through deduplicated browser attention notifications", () => {
  ...
  assert.match(
    completionSource,
    /tag: targetSession\s*\? `pi-session-complete:\$\{targetSession\.id\}`/,
  );
  assert.doesNotMatch(
    completionSource,
...
test("restores an in-page session viewport without the default tail jump", () => {
  ...
  assert.match(
    chatWindowSource,
    /isScrollAtTail\(\s*container\.scrollTop,\s*container\.clientHeight,\s*container\.scrollHeight,?\s*\)/,
  );
...
test("keeps prompt anchor measurement outside the React update cycle", () => {
  ...
  assert.match(
    chatWindowSource,
    /<div\s+ref=\{messageContentRef\}[^>]*style=\{\{/,
  );
});
