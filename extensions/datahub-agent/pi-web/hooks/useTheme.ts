...
function ensureState(): ThemeState {
  ...
  const preference = DATAHUB_EMBED ? "light" : readStoredPreference();
...
export function useTheme() {
  ...
  const setThemePreference = useCallback(
    (nextPreference: ThemePreference, origin?: ToggleOrigin) => {
      if (DATAHUB_EMBED) return; // Embedded theme follows deployment, without overwriting standalone preferences.
      const current = ensureState();
